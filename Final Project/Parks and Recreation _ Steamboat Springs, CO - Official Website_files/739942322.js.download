...
- Line 336: Parse error. syntax error
- Line 3078: Parse error. syntax error
- Line 3084: Parse error. syntax error
- Line 3105: Parse error. IE8 (and below) will parse trailing commas in array and object literals incorrectly. If you are targeting newer versions of JS, set the appropriate language_in option.
- Line 3145: Parse error. missing ; before statement
- Line 3145: Parse error. missing ; before statement
- Line 3145: Parse error. syntax error
- Line 3145: Parse error. missing ; before statement
- Line 3145: Parse error. illegal character
- Line 3145: Parse error. syntax error
- Line 3145: Parse error. illegal character
- Line 3145: Parse error. illegal character
*/
	...
	(function ($) {
		$.fn.cpModal = function (opt) {
			...
			this.show = function () {
				...
				$(document).on("keydown.cpModal." + $modal.attr('id'), function (e) {
					if (e.key === "Escape") {
						base.hide();
						return;
					}
					...
					var $focusableElements = $modal.find('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])').filter(':visible');
					...
					var first = $focusableElements.get(0);
					var last = $focusableElements.get($focusableElements.length - 1);
				...
				var $initialFocusableElement = $modal.find('a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])')
					.filter(':visible')
					.first();

				if ($initialFocusableElement.length > 0) {
			...
			this.hide = function () {
				...
				$(document).off("keydown.cpModal." + $modal.attr('id'));
...
function fetchSwitcherParams() {
    ...
    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            var url = "https://civicplussharedstorageqa.azureedge.net/preamble/CivicPlusSwitchers.js"
            if (!window.FeatureToggles.isActive("CMS.Site.NewAppSwitcher")) {
                url = "https://civicplussharedstorageqa.blob.core.windows.net/preamble/CivicPlusSwitchers.js";
            }
...
;
// session-timeout.js — improved and defensive version
// Test hook for shortening client-side timings without changing server defaults.
const sessionTimeoutConfig = window.CP_SESSION_TIMEOUT_CONFIG || {};

// How long before true expiry we warn the user and start the countdown modal.
const sessionTimeoutWarning = Number(sessionTimeoutConfig.warningMs) > 0
	? Number(sessionTimeoutConfig.warningMs)
	: 60 * 1000;

// Delay after popup refresh closes so updated auth cookies can propagate back to the opener tab.
const refreshRestartDelayMs = Number(sessionTimeoutConfig.refreshRestartDelayMs) > 0
	? Number(sessionTimeoutConfig.refreshRestartDelayMs)
	: 1500;

// Small grace period for transient login-cookie loss before treating the session as truly gone.
const missingLoggedInGraceMs = Number(sessionTimeoutConfig.missingLoggedInGraceMs) > 0
	? Number(sessionTimeoutConfig.missingLoggedInGraceMs)
	: 3000;

// How often to poll the refresh popup for closure when unload hooks are unreliable.
const popupPollIntervalMs = Number(sessionTimeoutConfig.popupPollIntervalMs) > 0
	? Number(sessionTimeoutConfig.popupPollIntervalMs)
	: 500;

// Shared localStorage key used to notify sibling tabs that the session was refreshed.
const SESSION_REFRESH_SIGNAL_KEY = "cp:sessionRefresh";

// Stable id for the session-timeout modal rendered by this script.
const SESSION_TIMEOUT_MODAL_ID = "cp-session-timeout-modal";

// Active one-second countdown interval for the warning modal.
let countdownTimer = null;

// Timer that waits until it is time to show the warning modal.
let popupTimer = null;

// Prevent duplicate timeout popups from opening in the same tab.
let popupOpen = false;

// --- Helper to replace AuthState.js dependency ---
function seedSessionTimeout(callback) {
	try {
		var xhr = new XMLHttpRequest();
		xhr.open('POST', '/MyAccount/SessionTimeout/GetAndResetServerTimeout', true);
		...
		xhr.onreadystatechange = function() {
			...
			try {
				...
				var timeoutSeconds = Number(response && response.Timeout);
				if (!timeoutSeconds) {
					callback(false);
					return;
				}

				const expirationTimestamp = Date.now() + (timeoutSeconds * 1000);
				const expirationDate = new Date(expirationTimestamp).toUTCString();
				document.cookie = `sessionExpiry=${expirationTimestamp}; expires=${expirationDate}; path=/; SameSite=Strict;`;
				callback(true);
			} catch (e) {
				callback(false);
			}
...
function verifyServerSession(callback) {
	// Simple XHR to avoid jQuery dependencies or race conditions
	try {
		var xhr = new XMLHttpRequest();
		xhr.open('GET', '/Authentication/IsLoggedIn?_=' + Date.now(), true);
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				if (xhr.status >= 200 && xhr.status < 300) {
					try {
						var r = JSON.parse(xhr.responseText);
						callback(!!(r && r.loggedIn));
					} catch (e) {
						callback(false);
					}
				} else {
...
function getPopupDelay(forceFresh) {
	// If forceFresh is true, we ignore the current SessionExpiry cookie (which might be stale/deleted)
	// and treat it as a brand new session start.
	const expiry = forceFresh ? null : readCookie("sessionExpiry");
	if (expiry) {
		const expiryTimestamp = Number(expiry);
		return Math.max(1, expiryTimestamp - Date.now() - sessionTimeoutWarning);
	}

	const timeoutCookie = readCookie("sessionTimeoutMinutes");
	const timeoutMinutes = Number(timeoutCookie);

	if (!timeoutMinutes) {
		console.warn("sessionTimeoutMinutes cookie missing or invalid! session timer disabled.");
		return 0;
	}

	const sessionTimeout = Math.max(1, timeoutMinutes) * 60; // seconds
	let expiresAfter = sessionTimeout * 1000;

	const expirationTimestamp = Date.now() + expiresAfter;

	const expirationDate = new Date(expirationTimestamp).toUTCString();

	document.cookie = `sessionExpiry=${expirationTimestamp}; expires=${expirationDate}; path=/; SameSite=Strict;`;

	return Math.max(1, expiresAfter - sessionTimeoutWarning);
}


function startSessionTimer(forceFresh) {
	// clear any existing timers (safe re-entry)
	if (popupTimer) {
	...
	if (!loggedInCookie || loggedInCookie.toLowerCase() !== "true") {
		document.cookie = `sessionExpiry=; expires=${new Date(0)}; path=/;`;
		return;
	} // not logged in — no timers needed

	if (!forceFresh && !readCookie("sessionExpiry") && !readCookie("sessionTimeoutMinutes")) {
		seedSessionTimeout(function(success) {
			if (!success) {
				console.warn("Unable to seed session timeout; session timer disabled.");
				return;
			}
	...
	const delayMilliseconds = getPopupDelay(forceFresh);
	if (!delayMilliseconds) {
		return;
	}
	popupTimer = setTimeout(showSessionTimeoutPopup, delayMilliseconds);
}
...
function readCookie(name) {
	// Escape cookie name for regex usage
	const match = document.cookie.match(new RegExp('(?:^|; )' + name.replace(/([.$?*|{}()\[\]\\\/])/g, '\\$1') + '=([^;]*)'));
	return match ? decodeURIComponent(match[1]) : null;
}

function broadcastSessionRefreshed() {
	// localStorage 'storage' event fires in *other* tabs.
	try {
		localStorage.setItem(SESSION_REFRESH_SIGNAL_KEY, String(Date.now()));
	} catch (e) {
		// ignore (storage disabled)
	}
}

function handleExternalSessionRefresh() {
	// Another tab refreshed the session; close popup and restart timers.
	stopCountdown();
	if (popupTimer) {
		clearTimeout(popupTimer);
		popupTimer = null;
...
function closeSessionTimeoutModal(event) {
	const sessionTimeoutModal = document.getElementById(SESSION_TIMEOUT_MODAL_ID);
	if (sessionTimeoutModal && sessionTimeoutModal.parentNode) {
		sessionTimeoutModal.parentNode.removeChild(sessionTimeoutModal);
	}
}

function renderSessionTimeoutModal(onRefresh, onLogout) {
	closeSessionTimeoutModal();

	const sessionTimeoutModal = document.createElement('div');
	sessionTimeoutModal.id = SESSION_TIMEOUT_MODAL_ID;
	sessionTimeoutModal.innerHTML = `
		<div style="position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:2147483646;"></div>
		<div style="position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); width:min(480px, calc(100vw - 32px)); background:#fff; color:#111; z-index:2147483647; border-radius:4px; box-shadow:0 8px 32px rgba(0,0,0,0.3); font-family:Arial, sans-serif;">
			<div style="padding:20px 24px 12px; font-size:24px; font-weight:600;">Session Timeout</div>
			<div style="padding:0 24px 16px; font-size:16px; line-height:1.5;">
				<p>Your session will expire in <span id="countdown"></span> seconds.</p>
			</div>
			<div style="display:flex; justify-content:flex-end; gap:12px; padding:0 24px 20px;">
				<button type="button" data-session-timeout-action="logout" style="min-width:120px; padding:10px 16px; border:1px solid #666; background:#fff; color:#222; cursor:pointer;">Log Out</button>
				<button type="button" data-session-timeout-action="refresh" style="min-width:140px; padding:10px 16px; border:1px solid #0057a8; background:#0057a8; color:#fff; cursor:pointer;">Refresh Session</button>
			</div>
		</div>`;

	document.body.appendChild(sessionTimeoutModal);

	const refreshButton = sessionTimeoutModal.querySelector('[data-session-timeout-action="refresh"]');
	const logoutButton = sessionTimeoutModal.querySelector('[data-session-timeout-action="logout"]');
...
function showSessionTimeoutPopup() {
	if (popupOpen) return; // already shown

	popupOpen = true;
	// When another tab is refreshing auth, the isLoggedIn cookie can briefly disappear.
	// Add a small grace window + server-backed check before forcing SignOut.
	let missingLoggedInSince = null;

	function handleRefreshSession(event) {
		// open a blank tab/window immediately on the user gesture
		const authWindow = window.open("/MyAccount/SessionTimeout/RefreshWindow", "Refresh Session", "status=1, height=600, width=500, resizable=0");
		if (!authWindow) {
			alert("Popup was blocked. Please allow popups for this site to open the page in a new tab, or try the action again.");
		} else {
			// Stop the countdown immediately so we don't logout while the user is refreshing
			stopCountdown();
			popupOpen = false;

			closeSessionTimeoutModal(event);

			authWindow.focus();

			// onbeforeunload/onunload do not work reliably after returning from the refresh redirect flow.
			// Instead, poll for popup closure.
			var authWindowPollInterval = setInterval(function () {
				if (authWindow.closed) {
					clearInterval(authWindowPollInterval);
					// Small delay to let the browser propagate cookies from the popup.
					setTimeout(function () {
						broadcastSessionRefreshed();
						startSessionTimer(true);
					}, refreshRestartDelayMs);
				}
			}, popupPollIntervalMs);
		}
	}
	...
	renderSessionTimeoutModal(handleRefreshSession, handleLogoutSession);

	let secondsRemaining = sessionTimeoutWarning / 1000;
	const countdownEl = document.getElementById("countdown");
	if (countdownEl) countdownEl.innerText = secondsRemaining;

	// ensure previous countdown cleared
	stopCountdown();

	countdownTimer = setInterval(function() {
		const expiry = readCookie("sessionExpiry");
		const loggedIn = readCookie("isLoggedIn");

		// Case 1: User logged out in another tab
		if (!loggedIn || loggedIn.toLowerCase() !== "true") {
			if (!missingLoggedInSince) missingLoggedInSince = Date.now();

			// Use our internal vanilla JS helper instead of window.AuthState
			verifyServerSession(function(isActuallyLoggedIn) {
				if (isActuallyLoggedIn) {
					missingLoggedInSince = null;
					return; // Session valid
				}

				if (Date.now() - missingLoggedInSince >= missingLoggedInGraceMs) {
					stopCountdown();
					popupOpen = false;
					expireLocalSession();
				}
			});
			return; // Pause execution for this tick while waiting for async response
		}
		...
		// Case 2: Session reached expiry while the warning modal is already open.
		// Browsers can drop the expiring cookie before our countdown text reaches zero,
		// so treat a missing/expired timestamp as a real timeout instead of a refresh.
		if (!expiry || !expiryTimestamp || expiryTimestamp <= Date.now()) {
			stopCountdown();
			popupOpen = false;
			expireLocalSession();
			return;
		...
		// Case 3: Session refreshed
		if (expiryTimestamp - Date.now() > sessionTimeoutWarning + 1000) {
			stopCountdown();
			popupOpen = false;
			closeSessionTimeoutModal();
			startSessionTimer(false); // Can be false here as we likely see the new cookie
			return;
		}

		secondsRemaining--;
		const el = document.getElementById("countdown");
		if (el) el.innerText = secondsRemaining;
...
function expireLocalSession() {
	window.location.href = '/MyAccount/SessionTimeout/Expire?txtRedirect=' + encodeURIComponent(window.location.pathname + window.location.search);
}

function explicitLogout() {
	window.location.href = '/publiclogin.aspx?out=true&txtRedirect=' + window.location.pathname.substring(1) + window.location.search;
}

// Prefer DOMContentLoaded if you only need the DOM (faster than load)
if (document.readyState === "complete" || document.readyState === "interactive") {
	// page already loaded — start immediately
	try { startSessionTimer(false); } catch (e) { console.error(e); }
} else {
...
// Cross-tab listener: another tab refreshed session.
// Note: the storage event only fires in other tabs, not the tab that called setItem.
window.addEventListener('storage', function (e) {
	if (!e) return;
	if (e.key !== SESSION_REFRESH_SIGNAL_KEY) return;
	handleExternalSessionRefresh();
});
;
var popOverModal = {
    open: function (title, bodyHtml, actions, options) {
        var model = {
            Open: true,
            Title: title,
            Body: {
    ...
    close: function (event) {
        ...
        document.getElementById('popoverModal').style.display = 'none';
        document.body.classList.remove('has-modalOpen');
    },
    render: function (model) {
        var modal = document.getElementById('popoverModal');
        modal.querySelector('.cp-PopOver-title').innerText = model.Title;
        modal.querySelector('.cp-PopOver-body').innerHTML = model.Body.Html;
        document.body.classList.add('has-modalOpen');
        modal.style.display = 'block';

        var actionButtonsContainer = document.getElementById('actionButtons');
        actionButtonsContainer.innerHTML = '';
        model.Actions.forEach(function (actionButton) {
            var button = document.createElement('button');
            button.className = 'cp-Btn ' + actionButton.ExtraClasses;
            button.innerText = actionButton.Title;
            button.onclick = function (event) {
                if (window[actionButton.fnClick]) {
                    window[actionButton.fnClick](event);
                } else {
                    actionButton.fnClick(event);
                }
            };
            button.disabled = actionButton.Disabled;
            actionButtonsContainer.appendChild(button);
        });
    }
};

function closePopOverModal(event) {
    popOverModal.close(event);
}
;
